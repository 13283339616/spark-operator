...
IMAGE_REPOSITORY ?= dongguiijun/spark-operator
IMAGE_TAG ?= $(VERSION)
...
deploy: manifests kustomize ## Deploy controller to the K8s cluster specified in ~/.kube/config.
	cd charts | helm install spark-operator spark-operator-chart/spark-operator --namespace spark-operator --create-namespace --wait

.PHONY: undeploy
undeploy: kustomize ## Undeploy controller from the K8s cluster specified in ~/.kube/config. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	helm uninstall spark-operator --namespace spark-operator --ignore-not-found=$(ignore-not-found)
